...
// 1. On importe l'entite/model "Notification"
...
export const createNotification = async (req, res) => {
    ...
    try {
        ...
        // Vérifier si l'utilisateur correspond à celui fourni dans les données
        if (utilisateur.id !== utilisateur_id) {
            return res.status(400).json({ message: "L'ID utilisateur ne correspond pas à celui de l'email" });
        }
...
export const createNotification = async (req, res) => {
    ...
    try {
        const { utilisateur_id, id_evenement, message, type_notification } = req.body;
        const notification = await Notification.create({
            utilisateur_id,
            id_evenement,
            message,
